const REPORT_UANG_MASUK = {
	...
	created : function(){
		const v = this;
		store.commit("changeNavTitle", "Report Penerimaan Kontribusi"); // JUDUL PAGE
	},
	mounted : function(){
		var v = this;
		v.get_session();
	...
	methods : {
        ...
        handleData : function(data_state){
            ...
            if(data_state == "SHOW"){
                ...
                $.ajax({
                    type : "GET",
                    url : BASE_URL+"report/report_uang_masuk",
                    data : v.forms,
                    success:function(resp){
                        if(resp.code == 200){
                            v.$set(v, "partList", resp.data);
                            v.$nextTick(() => {
                ...
                <table class="table table-bordered table-striped table-hover datatable">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center" width ="60px">Jumlah Transaksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="(item, index) in partList">
                            <td class="text-center">{{ index+1 }}</td>
                            <td>{{ item.nama_anggota }}</td>
                            <td>{{ item.provinsi }}</td>
                            <td>{{ item.kab_kota }}</td>
                            <td class="text-center">{{ item.TGL_SETOR }}</td>
                            <td class="text-right">{{ $root.formatNumber(item.nominal) }}</td>
                            <td class="text-center">{{ item.JUMLAH_KONTRIBUSI }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
