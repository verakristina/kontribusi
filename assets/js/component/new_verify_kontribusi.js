const VERIFY_KONTRIBUSI_NEW = {
	...
	methods : {
        getDataD : function(){
            ...
            $.ajax({
                type : "GET",
                url : BASE_URL+"api/get_verif_kontribusi_tanggal",
                data : {
        ...
        getDataDetail : function(){
            ...
            $.ajax({
                type : "GET",
                url : BASE_URL+"api/get_verif_kontribusi",
                data : {
                    unique_id : v.unique_id
                },
                success:function(resp){
            ...
            $.ajax({
                ...
                success: function(resp) {
                    if(resp.code == 200){
                        ...
                        window.location=BASE_URL+"verifikasi/kontribusi?month="+v.param.uri.month+"&year="+v.param.uri.year;
                    }else{
...
//new
    ...
    template : `
    <div>
        <router-link v-if="param.uri.with_save == ''" class="btn btn-danger" :to="{path : '/verifikasi/kontribusi?month='+param.uri.month+'&year='+param.uri.year}">
            <i class="fa fa-arrow-left"></i> Kembali
        </router-link>
        <router-link v-else class="btn btn-danger" :to="{path : '/verified/kontribusi?month='+param.uri.month+'&year='+param.uri.year}">
            <i class="fa fa-arrow-left"></i> Kembali
        </router-link>
        <div class="card">
            ...
            <table class="table table-bordered table-striped table-hover datatable">
                ...
                <tbody>
                    <tr v-for="(item, index) in detail">
                        ...
                        <td>{{$root.formatNumber (item.dpp) }}</td>
                        <td>{{$root.formatNumber (item.dpd) }}</td>
                        <td>{{$root.formatNumber (item.dpc) }}</td>
                    </tr>
                </tbody>
            </table>
            </div>
        ...
        <div class="card">
            <div class="card-header card-header-icon card-header-warning">
                ...
                <div class="card-title">
                    Verifikasi Bukti Setor Dana Kontribusi
                </div>
            </div>
            <div class="card-body">
                <div class="row my-2">
                ...
                <table class="table">
                    ...
                    <tr v-for="(it, ind) in file">
                        ...
                        <td>{{ it.SETOR }}<br>{{ $root.formatNumber(it.nominal) }}</td>
                        <td>
                            <input type="file" 
                                ref="file" 
                                class="dropify" 
