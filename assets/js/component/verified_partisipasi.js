const PAGE_VERIFIED_PARTISIPASI = {
	data : function() {
		return {
            userdata : [],
            dataList : {
	...
	created : function(){
		const v = this;
        store.commit("changeNavTitle", "Dana Partisipasi Terverifikasi"); // JUDUL PAGE
	...
	methods : {
        ...
        getVerifikasiList : function(){
            ...
            $.ajax({
                type: "GET",
                url : BASE_URL+"api/get_verified_partisipasi_list",
                data : {
                    bulan : v.forms.bulan,
                    tahun : v.forms.tahun
                },
                ...
                <table class="table table-bordered table-striped table-hover datatable">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center" width="150px">Tanggal Verifikasi</th>
                    ...
                    <tbody>
                        <tr v-for="(item, index) in dataList.verification_list">
                            ...
                            <td class="text-center">{{ item.TGL_VERIF }}</td>
                            <td>{{ item.nama_anggota }}</td>
                            <td class="text-left">{{ item.provinsi }}</td>
                            <td class="text-left">{{ item.kab_kota }}</td>
                            <td class="text-center">{{ $root.formatNumber(item.dana_partisipasi) }}</td>
                            <td class="text-center">
                                <router-link class="btn btn-success btn-sm" :to="{ path: '/verify-partisipasi/'+item.id+'?month='+forms.bulan+'&year='+forms.tahun+'&with_save=false' }">
