const PAGE_VERIFIED_KONTRIBUSI = {
	data : function() {
		return {
            userdata : [],            
            unique_id : "",
	...
	created : function(){
		const v = this;
        store.commit("changeNavTitle", "Dana Kontribusi Terverifikasi"); // JUDUL PAGE
	...
	methods : {
        ...
        getVerifikasiList : function(){
            ...
            $.ajax({
                type: "GET",
                url : BASE_URL+"api/get_verified_kontribusi_list",
                data : {
                    ...
                    <tbody>
                        <tr v-for="(item, index) in dataList.verification_list">
                            ...
                            <td class="text-center">
                                <router-link class="btn btn-success btn-sm" :to="{ path: '/verify/'+item.unique_identifier+'?month='+forms.bulan+'&year='+forms.tahun+'&with_save=false' }">
